let templateCodeGen = {
genSql: (template,param) =>{
    console.log("genSql,template:"+template + "param:" + param)
    ...
    paramLineArr.forEach(item =>{
        if(item.indexOf("-") != -1){
            let parseArr = templateCodeGen.genParamForDash(item)
            console.log(item)
            //将解析后的数组合入原数组
    ...
    parseParamLineArr.forEach(element => {
        ...
        if(element.indexOf(",") != -1){
            ...
            //处理转义符
            tmpArr = templateCodeGen.parseParamEscape(tmpArr)
            resultArr.push(stringformat(template, ...tmpArr))
        }else{
            resultArr.push(stringformat(template,element))
        }
    })
    return resultArr.join("\n")
},

genParamForDash:(param) =>{
    ...
    let numArr = []
    if(tempArr.length>1){
        if(!templateCodeGen.isInteger(tempArr[0]) || !templateCodeGen.isInteger(tempArr[1])){
            //如果-前后不是数字,原样输出
